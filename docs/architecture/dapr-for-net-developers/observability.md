...
In the figure, note how the trace represents a unique application transaction that takes places across many services. A trace is a collection of *spans*. Each span represents a single operation or unit of work done within the trace. Spans are the requests and responses that are sent between services that implement the unique transaction.
...
Once the application is started, the Dapr sidecars will emit telemetry to the Zipkin server. To inspect this telemetry, point a web-browser to [http://localhost:32411](http://localhost:32411). You'll see the Zipkin webpage, as shown in Figure 9-2:
...
Each item on the details page, represents a span that is part of an operation in the selected trace.
...
Metrics provide insight into performance and resource consumption. Under the hood, Dapr emits a wide collection of system and runtime metrics. Dapr uses [Prometheus](https://prometheus.io/) as a metric standard. Dapr sidecars and internal processes expose a metrics endpoint on port `9090`. A *Prometheus scraper* calls this endpoint at a predefined interval to collect metrics. The scraper sends metric values to a monitoring backend. Figure 9-6 shows the scraping process:

![Scraping Prometheus metrics ](media/observability/prometheus-scraper.png)

**Figure 9-6**: Scraping Prometheus metrics

In the above figure, each sidecar and process expose a metric endpoint that listens on port 9090. The Prometheus Metrics Scrapper captures metrics from each endpoint and published the information to the monitoring backend.  
